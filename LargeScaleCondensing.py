...
parser.add_argument('--aggr_layer', type=int, default=1)
...
parser.add_argument('--lr_model', type=float, default=0.002)
...
parser.add_argument('--threshold', type=float, default=1.00, help='adj threshold.')
...
def edge_construction():
    ...
    if args.edge_pred=='aggr':#find the anchors，directly use their feat_transform
        feat_syn_neighbor = torch.zeros(feat_syn.shape[0],(args.aggr_layer)*d).cuda()
        ...
        for c in range(nclass):
            if c in num_class_dict:
                ...
                feat_syn_neighbor[index_syn[c]] = feat_transform[index[c]][anchor,:(args.aggr_layer)*d].max(dim=1).values
...
if __name__ == '__main__':
    ...
    if args.edge_pred in ['aggr']:
        pge_edge = PGE_Edge(nfeat=(1+args.aggr_layer)*d, device=device, args=args).cuda()
    else:
