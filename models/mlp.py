...
class MLP(torch.nn.Module):
    ...
    def forward(
        ...
        x=(x, emb) if isinstance(return_emb, bool) else x
    ...
    @torch.no_grad()
    def inference(self, x_all: Tensor, batch_size: int, return_emb: NoneType = None) -> Tensor:
        self.eval()
        xs: List[Tensor] = []

        for i in range(x_all.shape[0]//batch_size+1):
            x = x_all[i * batch_size : min((i+1)*batch_size, x_all.shape[0])]
            for i, (lin, norm) in enumerate(zip(self.lins, self.norms)):
                x = lin(x)
                if self.act is not None and self.act_first:
                    x = self.act(x)
